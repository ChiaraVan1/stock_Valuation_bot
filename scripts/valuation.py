...
新增：支持结构化输入文件（input/{code}.json），
      提供精确的历史数据，优先级高于OCR提取。
"""
...
def load_input_json(code: str, input_file: str = "") -> dict:
    ...
    for path in candidates:
        if path.exists():
            ...
            print(f"✅ 已加载结构化输入文件: {path}")
            return data

    print(f"ℹ️  未找到结构化输入文件（input/{code}.json），将完全依赖OCR提取")
    return {}


def format_input_override(input_data: dict) -> str:
    """
    将结构化输入文件的内容格式化为注入DeepSeek prompt的文字块。
    优先级最高，DeepSeek必须使用这里的数字，不得被OCR数据覆盖。
    """
    if not input_data:
        return ""
    ...
    lines = [
        ...
        "DeepSeek在估值计算中必须使用以下数字，若与OCR数据冲突，以本节为准。",
        "",
    ]
    ...
    if "net_profit_history" in input_data:
        ...
        lines.append("")
        lines.append("- **近年归母净利润序列（亿元）**⭐轨道一三情景计算使用此序列，不得从截图重新推算：")
        for year in sorted(np_hist.keys()):
            lines.append(f"  - {year}年: {np_hist[year]}亿")
        total = sum(np_hist.values())
        lines.append(f"  - 合计: {total:.2f}亿（{min(np_hist.keys())}-{max(np_hist.keys())}年）")

    # 历史分红序列
    if "dividend_history" in input_data:
        div_hist = input_data["dividend_history"]
        lines.append("")
        lines.append("- **历年现金分红序列（亿元，不含回购）**⭐分红率计算使用此序列：")
        for year in sorted(div_hist.keys()):
            lines.append(f"  - {year}年: {div_hist[year]}亿")
        total_div = sum(div_hist.values())
        lines.append(f"  - 合计: {total_div:.2f}亿（{min(div_hist.keys())}-{max(div_hist.keys())}年）")

    lines.append("")
    lines.append("> **执行指令**：上述数据已由用户人工核验。")
    lines.append("> 轨道一的分红率、三情景盈利力、每股计算，必须基于以上序列，不得自行从截图重新读取或推算。")
    lines.append("")
...
def validate_ocr_data(financial_data: str, client) -> dict:
    ...
    if ta and tl:
        ratio = tl / ta
        if ratio > 0.8:
            errors.append(f"负债合计({tl:.2f}亿)占总资产({ta:.2f}亿)的{ratio:.1%}，超过80%上限，可能OCR读错")
        else:
    ...
    if pe and me and te:
        diff = abs(pe + me - te)
        if diff > te * 0.01:
            warnings.append(f"归母权益({pe:.2f}) + 少数股东({me:.2f}) ≠ 权益合计({te:.2f})，差额{diff:.2f}亿")
...
def format_validation_report(validation: dict) -> str:
    """将OCR校验结果格式化为注入DeepSeek prompt的文字块（辅助数据层）"""
...
研报内容："""
        ...
        for pdf_path in pdf_files[:max_reports]:
            ...
            text = text[:10000]
            resp = client.chat.completions.create(
                model="deepseek-v3.2", max_tokens=300, temperature=0,
...
def main():
    ...
    parser.add_argument("--input-file", required=False, default="",
                        help="结构化输入文件路径（如 input/002223.json），提供精确历史数据")
