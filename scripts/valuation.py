...
**【催化剂核查清单】（成长板块溢价的执行框架）**
对每个增速 ≥15% 的板块，在给出溢价前必须逐项回答以下问题，并以表格形式输出：

| 核查项 | 问题 | 结论（是/否/不确定） | 证据来源 |
|--------|------|---------------------|---------|
| C1 产品准入 | 报告期内是否有新产品注册证/CE认证/FDA准入落地？ | | |
| C2 海外准入 | 是否有新海外市场准入或经销协议签署？ | | |
| C3 重大合同 | 是否有单笔金额超过年营收 5% 的合同公告？ | | |
| C4 政策催化 | 是否有明确利好该板块的政策文件发布？ | | |
| C5 研报点名 | 是否有券商研报将该板块单独列为核心看点？ | | |

溢价幅度与核查结果的对应关系：
- C1~C5 均为否/不确定 → 溢价 0x，与成熟板块统一使用整体 PE
- 满足 C1 或 C2 任意一项 → 溢价上限 +1x
- 满足（C1 且 C2）或 C5 → 溢价上限 +2x（硬上限，不因催化剂数量叠加突破）

**无论溢价幅度，必须在报告中写明触发溢价的具体事件名称和来源。若无法提供具体事件，溢价归零。**
...
def _run_balance_sheet_validation(
    cash: float, cash_prior: float,
    other_current: float, other_noncurrent: float,
    current_due_noncurrent: float, trading_assets: float,
    fixed_assets: float, goodwill: float, total_assets: float,
) -> tuple[str, float | None]:
    """
    运行资产负债表链路层校验，返回：
      - validation_note: 注入 text_intro 的校验摘要
      - cash_merged: 触发骤降规则时的合并现金类资产（亿元），否则为 None
    """
    bs = BalanceSheet(
        cash=cash,
        cash_prior=cash_prior if cash_prior > 0 else None,
        other_current=other_current,
        other_noncurrent=other_noncurrent,
        current_due_noncurrent=current_due_noncurrent,
        trading_assets=trading_assets,
        fixed_assets=fixed_assets,
        goodwill=goodwill,
        total_assets=total_assets if total_assets > 0 else 1.0,
    )
    result = validate_balance_sheet(bs)

    lines = ["## ⚙️ 链路层预校验（在 Claude 估值前已执行）\n"]
    if result.errors:
        lines.append("### ❌ 阻断性错误")
        lines += [f"- {e}" for e in result.errors]
        lines.append("")
    if result.warnings:
        lines.append("### ⚠️ 警告（须在报告中标注）")
        lines += [f"- {w}" for w in result.warnings]
        lines.append("")
    if result.cash_merged is not None:
        lines.append(
            f"### 现金类资产合并结果\n"
            f"合并后现金类资产 = **{result.cash_merged:.2f}亿**\n"
            f"{result.cash_merge_detail}\n"
            f"> **两轨净现金均须使用此合并值 {result.cash_merged:.2f}亿，不得单独使用货币资金。**\n"
        )
    ...
    if not result.passed:
        raise SystemExit(
            f"❌ 链路层校验失败，估值已阻断。请修正以下错误后重新运行：\n"
            + "\n".join(result.errors)
        )

    return "\n".join(lines), result.cash_merged


def _check_scenario_monotonic(report: str) -> str:
    """
    从 Claude 输出的报告文本中解析三情景数值并做单调性断言。
    违反时在报告头部插入红色 banner，保留报告内容便于排查。
    返回（可能已加 banner 的）报告文本。
    """
    patterns = [
        (r"悲观[^\n|]*\|\s*[\d.]+\s*\|\s*([\d.]+)",
         r"中性[^\n|]*\|\s*[\d.]+\s*\|\s*([\d.]+)",
    ...
    scenarios = None
    for p_b, p_n, p_o in patterns:
        mb = re.search(p_b, report)
        mn = re.search(p_n, report)
        mo = re.search(p_o, report)
        if mb and mn and mo:
            try:
                scenarios = Scenarios(
                    bearish=float(mb.group(1)),
                    neutral=float(mn.group(1)),
                    optimistic=float(mo.group(1)),
                )
                break
            except ValueError:
                continue
    ...
    if scenarios is None:
        print("⚠️  无法从报告中解析三情景数值，跳过单调性校验")
        return report

    try:
        assert_scenario_monotonic(scenarios)
        print(f"✅ 三情景单调性校验通过（{scenarios.bearish} < {scenarios.neutral} < {scenarios.optimistic}）")
    except ScenarioMonotonicError as e:
        banner = (
            "\n\n> ⛔ **链路层拦截：三情景单调性校验失败，请人工核查后再使用本报告**\n"
...
def run_valuation(
    ...
    report_md: str = "",
    # ── PR1 新增：链路层校验参数（全部可选，不填则跳过校验）──
    cash: float = 0.0,
    cash_prior: float = 0.0,
    other_current: float = 0.0,
    other_noncurrent: float = 0.0,
    current_due_noncurrent: float = 0.0,
    ...
    # ── PR1 新增：Step2.5 链路层资产负债表校验 ──────────────
    validation_note = ""
    if cash > 0 or total_assets > 0:
        print("\n--- Step2.5: 链路层资产负债表校验 ---")
        validation_note, cash_merged = _run_balance_sheet_validation(
            cash=cash, cash_prior=cash_prior,
            other_current=other_current, other_noncurrent=other_noncurrent,
            current_due_noncurrent=current_due_noncurrent,
            trading_assets=trading_assets,
            fixed_assets=fixed_assets, goodwill=goodwill,
            total_assets=total_assets,
        )
        if cash_merged is not None:
            print(f"  → 合并现金类资产: {cash_merged:.2f}亿")
    else:
        print("\n--- Step2.5: 链路层校验跳过（未传入资产负债表参数）---")
    # ─────────────────────────────────────────────────────────
...
def main():
    parser = argparse.ArgumentParser(description="双轨估值分析 v8（直接传图 + 链路层校验）")
    ...
    # ── PR1 新增：链路层校验参数 ──
    parser.add_argument("--cash",        type=float, default=0.0, help="货币资金（亿元）")
    parser.add_argument("--cash-prior",  type=float, default=0.0, help="上期货币资金（亿元），用于骤降检测")
    parser.add_argument("--other-current",      type=float, default=0.0, help="其他流动资产（亿元）")
    parser.add_argument("--other-noncurrent",   type=float, default=0.0, help="其他非流动资产（亿元）")
    parser.add_argument("--current-due-noncur", type=float, default=0.0, help="一年内到期非流动资产（亿元）")
    parser.add_argument("--trading-assets",     type=float, default=0.0, help="交易性金融资产（亿元）")
    parser.add_argument("--fixed-assets",       type=float, default=0.0, help="固定资产（亿元）")
    parser.add_argument("--goodwill",           type=float, default=0.0, help="商誉（亿元）")
    parser.add_argument("--total-assets",       type=float, default=0.0, help="资产总计（亿元）")
    args = parser.parse_args()

    print(f"🚀 双轨估值 v8（直接传图 + 链路层校验）| {args.name}（{args.code}）{args.year}年报")
    run_valuation(
        ...
        cash=args.cash, cash_prior=args.cash_prior,
        other_current=args.other_current, other_noncurrent=args.other_noncurrent,
        current_due_noncurrent=args.current_due_noncurrent,
        trading_assets=args.trading_assets,
        fixed_assets=args.fixed_assets, goodwill=args.goodwill,
