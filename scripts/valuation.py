...
双轨估值系统 - 完整版 v4
★ 改动：
  - validate_ocr_data()  → Claude优先（OCR校验，财务逻辑判断更准）
  - run_valuation Step3  → Claude优先（主估值，复杂规则不漏步骤）
  - Step1图片OCR         → 保留千问VL（中文财报图片识别专项优化）
  - 所有AI调用统一走 ai_client.text_completion()
"""
...
def load_input_json(code: str, input_file: str = "", name: str = "") -> dict:
    ...
    for path in candidates:
        if path.exists():
            ...
            print(f"✅ 已加载结构化输入: {path}")
            return data
    print("ℹ️  未找到结构化输入文件，完全依赖OCR")
    return {}


def format_input_override(input_data: dict) -> str:
    if not input_data:
        return ""
    lines = [
        "## ⭐ 用户提供的精确数据（最高优先级，直接使用，不得被OCR数据覆盖）", "",
        "以下数据由用户手动填写，经过人工核验，准确性高于OCR提取。",
        "在估值计算中必须使用以下数字，若与OCR数据冲突，以本节为准。", "",
    ]
    if "shares" in input_data:
        lines.append(f"- **总股本**: {input_data['shares']} 亿股 ⭐所有每股计算使用此值")
    if "liabilities" in input_data:
        lines.append(f"- **负债合计**: {input_data['liabilities']} 亿元 ⭐清算价值计算使用此值")
    if "net_profit_history" in input_data:
        np_hist = input_data["net_profit_history"]
        lines += ["", "- **近年归母净利润序列（亿元）**⭐轨道一三情景计算使用此序列："]
        for year in sorted(np_hist.keys()):
            lines.append(f"  - {year}年: {np_hist[year]}亿")
        lines.append(f"  - 合计: {sum(np_hist.values()):.2f}亿")
    if "dividend_history" in input_data:
        div_hist = input_data["dividend_history"]
        lines += ["", "- **历年现金分红序列（亿元，不含回购）**⭐分红率计算使用此序列："]
        for year in sorted(div_hist.keys()):
            lines.append(f"  - {year}年: {div_hist[year]}亿")
        lines.append(f"  - 合计: {sum(div_hist.values()):.2f}亿")
    lines += ["", "> **执行指令**：上述数据已由用户人工核验，估值计算必须基于以上序列。", ""]
    return "\n".join(lines)


# ────────────────────────────────────────────────────────────
# Step1：千问VL图片OCR（保持原样，不替换为Claude）
# ────────────────────────────────────────────────────────────
        ...
        resp = client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {"type": "text", "text": """请仔细识别这张财务截图中的所有数据，按以下格式输出：

1. 截图类型（如：资产负债表/利润表/归母净利润历史/分红记录/分产品营收/股本信息/股价信息等）
2. 所有数字数据，保持原始格式，注明单位
3. 表格数据请逐行列出

要求：只输出数据，不要分析，不要遗漏任何数字。若截图中同时存在合并报表和母公司报表，只提取合并报表数据，忽略母公司报表，并在输出开头标注"已提取：合并报表"。若截图包含资产负债表，必须提取"股本（实收资本）"和"负债合计"两行数字，这两项不得遗漏。"""}
                ]
            }]
        )
...
输出格式：
{
  ...
  "cash_consolidated": 数字（亿元，合并资产负债表货币资金，必须取合并口径）
}

注意：
- total_liabilities 通常远小于 total_assets，约为 total_assets 的 10%-40%
- net_profit_consolidated 是合并报表数字
- 若发现 net_profit_parent 与 net_profit_consolidated 差异超过30%，务必两个都填
...
def validate_ocr_data(financial_data: str) -> dict:
    """
    ★ OCR数据校验改用Claude优先。
    Claude对资产负债表恒等式、口径混用等财务逻辑判断更准确。
    """
    ...
    if ta and tl and te:
        ...
        if diff > ta * 0.01:
            errors.append(
                f"资产负债表恒等式不成立：总资产({ta:.2f}) ≠ 负债({tl:.2f}) + 权益({te:.2f})，差额{diff:.2f}亿"
            )
        else:
    ...
    if np_c and np_p:
        ...
        if diff_pct > 0.3:
            warnings.append(
                f"合并净利润({np_c:.2f}亿) 与母公司净利润({np_p:.2f}亿) 差异{diff_pct:.1%}，请确认使用合并口径"
            )
...
def format_validation_report(validation: dict) -> str:
    ...
    lines = ["## OCR数据校验结果（Step1.5自动生成，优先级低于用户提供数据）", ""]
    if extracted:
        lines.append("### 关键数字（OCR提取，若与用户提供数据冲突以用户数据为准）")
        fields = [
            ("total_assets",            "总资产（亿元）"),
            ...
            ("total_shares",            "总股本（亿股）"),
            ("cash_consolidated",       "货币资金-合并口径（亿元）"),
        ]
    ...
    if errors:
        lines += ["### 🚨 OCR严重错误"] + [f"- ❌ {e}" for e in errors] + [""]
    if warnings:
        lines += ["### ⚠️ OCR警告"] + [f"- {w}" for w in warnings] + [""]
    if not errors and not warnings:
        lines += ["### ✅ OCR校验通过", ""]
...
# ────────────────────────────────────────────────────────────
# Step2：研报倍数（保持原逻辑，内部已由fetch_and_extract改用Claude）
# ────────────────────────────────────────────────────────────
...
def download_and_extract_reports(code: str, months: int, max_reports: int) -> str:
    """fallback模式：直接在valuation.py内下载+提取（不依赖外部md文件）"""
    ...
    import pdfplumber
    from ai_client import text_completion as _tc
...
以下是从截图中提取的所有财务数据（优先级低于上方用户提供数据）：
...
请根据以上数据完成完整的双轨估值分析，输出完整报告（Markdown格式）。"""
    ...
    report, provider = text_completion(
        ...
        max_tokens=4000,          # DeepSeek fallback上限
        claude_max_tokens=8000,   # Claude给更多token，规则复杂
        temperature=0.1,
        task_label="主估值分析",
...
def main():
    parser = argparse.ArgumentParser(description="双轨估值分析 v4")
    ...
    parser.add_argument("--input-file",  required=False, default="",
                        help="结构化输入文件路径（如 input/002223.json）")
    args = parser.parse_args()

    print(f"🚀 双轨估值 v4 | {args.name}（{args.code}）{args.year}年报")
    run_valuation(
        ...
        report_md=args.report_md, input_file=args.input_file,
    )
