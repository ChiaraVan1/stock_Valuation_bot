...
### 第一步：识别分部数据
从截图中提取每个独立业务板块的营收金额与毛利率（过滤汇总行、合计行、"其他业务"）
...
### 第三步：SOTP 计算
- 板块净利润 = 板块营收 × 整体净利润率（若有分部毛利率则按毛利率加权分配）
- PE 板块估值 = 板块净利润 × 悲观 PE
...
请根据以下材料完成完整的双轨估值分析，输出完整报告（Markdown格式）：
"""
...
def load_images(code: str, name: str = "") -> list[dict]:
    """读取股票对应文件夹下的所有截图，转为base64。先找代码，找不到再找名称"""
    ...
    for img_path in sorted(img_dir.iterdir()):
        ...
        print(f"  已加载截图: {img_path.name}")

    print(f"共加载 {len(images)} 张截图")
    return images
...
def run_valuation(code: str, stock_name: str, report_year: str,
    ...
    # 加载截图
    print(f"\n📸 加载截图...")
    images = load_images(code, stock_name)

    # 下载研报倍数
    print(f"\n📊 获取研报估值倍数...")
    report_table = download_reports(code, months, max_reports)
    print(report_table[:300])

    # 构建用户消息（截图 + 文字说明）
    print(f"\n🤖 调用 DeepSeek 进行双轨估值分析...")
...
请按照你的双轨估值分析框架，完整输出轨道一、轨道二、交叉验证、局限性说明和风险提示。"""
        }
    ]
