...
修复记录：
  - EXTRACT_PROMPT 由 f-string 改为普通字符串拼接，
    避免 JSON 模板中的 {} 被 Python 解释器当成插值表达式，
    触发 ValueError: Invalid format specifier。
"""
...
# ── 修复核心：EXTRACT_PROMPT 改为普通字符串，不使用 f-string ──
# 原写法：EXTRACT_PROMPT = f"""..."""
# 问题：JSON 模板中的 {"key": "value"} 被 Python 当成 f-string 插值，触发 ValueError
# 修复：规则部分在函数 build_extract_prompt() 中动态拼接，JSON 模板用普通字符串
# ──────────────────────────────────────────────────────────────

_EXTRACT_PROMPT_PREFIX = """你是A股财务数据提取专员。从以下年报截图中提取所有财务数据，输出严格JSON。
...
def build_extract_prompt() -> str:
    """动态拼接完整的 EXTRACT_PROMPT，避免 f-string 花括号冲突。"""
    return _EXTRACT_PROMPT_PREFIX + _get_extraction_rules() + _EXTRACT_PROMPT_SUFFIX
...
def main():
    ...
    # 动态构建 prompt（此处拼接，不在模块级执行，避免 import 时报错）
    ...
    resp = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user",    "content": image_blocks + [{"type": "text", "text": extract_prompt}] + [text_block]}
        ]
    )
